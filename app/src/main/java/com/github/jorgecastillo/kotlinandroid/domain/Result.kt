package com.github.jorgecastillo.kotlinandroid.domain

import com.github.jorgecastillo.kotlinandroid.domain.Result.Factory.pure
...
/**
 * A Result is a deferred promise containing either a controlled error and successful value or an
 * unknown exception. The containing Disjunction is right biased on A.
 */
class Result<E, A>(private val value: Promise<Disjunction<E, A>, Exception>) {
  ...
  /**
   * Shortcut swap to the disjunction when promise completes
   */
  ...
  companion object Factory {
    ...
    private fun <E, A, B> foldNext(iterator: Iterator<Result<E, A>>, zero: B,
        f: (B, A) -> B): Result<E, B> =
