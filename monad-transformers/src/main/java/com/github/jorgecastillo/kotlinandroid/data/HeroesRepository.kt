...
fun getHeroes(policy: CachePolicy): AsyncResult<List<CharacterDto>> = when (policy) {
...
fun getHeroDetails(policy: CachePolicy, heroId: String): AsyncResult<List<CharacterDto>> = when (policy) {
...
fun getHeroesFromAvengerComics(policy: CachePolicy): AsyncResult<List<CharacterDto>> = when (policy) {
