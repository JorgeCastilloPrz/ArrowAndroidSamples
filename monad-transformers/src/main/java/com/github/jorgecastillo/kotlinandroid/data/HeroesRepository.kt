...
fun <D: SuperHeroesContext> getHeroDetails(policy: CachePolicy, heroId: String): AsyncResult<D, CharacterDto> = when (policy) {
...
fun <D: SuperHeroesContext> getHeroesFromAvengerComics(policy: CachePolicy): AsyncResult<D, List<CharacterDto>> = when (policy) {
